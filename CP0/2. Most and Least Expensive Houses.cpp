...
int main() {
	...
	int max = INT_MIN;
	int min = INT_MAX;
	int hp1 = INT_MIN, hb1, hsf1;
	int hp2 = INT_MAX, hb2, hsf2;
...
//500000 5 3000
		...
		if ((bed >= 2 && bed <= 3) && (max < price))
			cerr << price << endl;
		max = price, hp1 = price, hb1 = bed, hsf1 = sf;


		if ((bed >= 4 && bed <= 5) && (min > price)) {
			min = price, hp2 = price, hb2 = bed, hsf2 = sf;
		}
	...
	cout << hp1 << " " << hb1 << " " << hsf1 << endl;
	cout << hp2 << " " << hb2 << " " << hsf2 << endl;
}