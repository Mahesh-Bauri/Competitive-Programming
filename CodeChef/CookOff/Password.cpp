...
void solve() {
	...
	bool isLower = false, isUpper = false, isDigit = false, isSpec = false;
	int size = (int)s.size();
	bool isSize = false;
	...
	for (int i = 0; i < size; i++) {
		...
		// if (s[i] >= 'a' && s[i] <= 'z') isLower = true;
		// if (i != 0 && i != size - 1) {
		// 	if (s[i] >= 'A' && s[i] <= 'Z') isUpper = true;
		// 	if (s[i] >= '0' && s[i] <= '9') isDigit = true;
		// 	if (s[i] == '@' || s[i] == '#' || s[i] == '%' || s[i] == '&' || s[i] == '?') isSpec = true;
		// }

	}


	if (isSize && isSpec && isDigit && isUpper && isLower ) {
		cout << "YES" << endl;
	}
	else {
		cout << "NO" << endl;
