...
void solve() {
	...
	if (abs(0 - minIndex) + 1 < abs((n - 1) - minIndex) + 1) {
		minDist = abs(0 - minIndex) + 1;
	} else {
		minDist = abs((n - 1) - minIndex) + 1;
	}

	int maxDist;

	if (abs(0 - maxIndex) + 1 < abs((n - 1) - maxIndex) + 1) {
		maxDist = abs(0 - maxIndex) + 1;
	} else {
		maxDist = abs((n - 1) - maxIndex) + 1;
	}

	int d1 = minDist + maxDist;
	int d2 ;
	int d3 ;
	...
	if (minIndex < maxIndex) {
		d3 = abs((n - 1) - minIndex) + 1;
	} else {
		d3 = abs((n - 1) - maxIndex) + 1;
	}
